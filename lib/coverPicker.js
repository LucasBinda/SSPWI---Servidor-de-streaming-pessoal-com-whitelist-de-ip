...
const { MOVIES_DIR, COVERS_DIR, CATALOG_PATH } = require('./paths');
...
class CoverPicker {
  ...
  processarProxima() {
    ...
    this.gerarCapa(rel)
      .then(() => {
        ...
        logManager.info('capas', `capa gerada para ${path.basename(rel)}`);
      })
      .catch((err) => {
        logManager.registrarErro('capas', `falha ao gerar capa de ${rel}: ${err.message}`);
      })
      .finally(() => {
  ...
  atualizarCatalogo(rel, capaUrl) {
    // Default null (não []): catálogo ilegível no momento -> não faz nada, em
    // vez de reescrever por cima de um arquivo que só falhou de ler agora.
    const lista = lerJson(this.catalogPath, null);
    ...
    item.capa = capaUrl;
    try {
      salvarJson(this.catalogPath, lista);
    } catch (err) {
      logManager.registrarErro('capas', `falha ao atualizar catálogo: ${err.message}`);
    }
  }
  ...
  removerCapa(rel) {
    fs.rm(this.capaAutoAbs(rel), { force: true }, (err) => {
      if (err) logManager.registrarErro('capas', `falha ao remover capa de ${rel}: ${err.message}`);
    });
  }

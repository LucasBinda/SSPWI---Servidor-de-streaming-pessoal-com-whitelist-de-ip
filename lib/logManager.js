...
// Janela de deduplicação dos bloqueios (por IP + motivo). Um cliente barrado
// entra num laço de reautorização (polling de 20s + reload) que dispara o
// MESMO bloqueio sem parar — sem dedup, isso enche o log com linhas idênticas
// (foi o que aconteceu com o cookie Secure sobre HTTP). Dentro da janela, o
// mesmo IP+motivo vira uma linha só; motivo diferente, ou o mesmo bem depois,
// registra de novo — a informação de "está acontecendo" fica, o spam não.
const JANELA_DEDUP_BLOQUEIOS_MS = 30 * 60 * 1000;
...
class LogManager {
  constructor({ logsDir }) {
    ...
    this.ultimoBloqueio = new Map(); // "ip|motivo" -> timestamp
  }
  ...
  // mapa cresce demais. Compartilhado por conexão, chamada e bloqueio.
  deveRegistrar(mapa, chave, janelaMs) {
    // antiFilterLog liga o "modo verboso": desliga o dedupe pra TODOS os
    // registros (conexão/chamada/bloqueio) — cada linha é escrita, sem
    // colapsar repetições. `=== true` estrito: só o booleano literal ativa;
    // ausente/inválido segue deduplicando (padrão seguro).
    if (loadSettings().antiFilterLog === true) return true;
  ...
  // tentou entrar, no console (amarelo) e em logs/bloqueios.log. Deduplicada
  // por IP+motivo (JANELA_DEDUP_BLOQUEIOS_MS): um cliente preso no laço de
  // reautorização repete o mesmo bloqueio a cada 20s — vira uma linha só por
  // janela, em vez de encher o log. Motivo diferente conta como evento novo.
  registrarBloqueio(ip, motivo) {
    if (!this.deveRegistrar(this.ultimoBloqueio, `${ip}|${motivo}`, JANELA_DEDUP_BLOQUEIOS_MS)) return;

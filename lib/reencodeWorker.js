...
const {
  MOVIES_DIR,
  CATALOG_PATH,
  REENCODE_STATE_PATH: STATE_PATH,
...
function lerEstado() {
  return lerJson(STATE_PATH, { arquivos: {} }, (e) => e && typeof e === 'object' && e.arquivos && typeof e.arquivos === 'object');
}

function salvarEstado(estado) {
  try {
    salvarJson(STATE_PATH, estado);
  } catch (err) {
...
async function escolherModo(origem, codecOrigem, duracaoSegundos, settings, rel) {
  const candidatos = ['software', 'nvenc'];
  if (NVDEC_CODECS.has(codecOrigem)) candidatos.push('nvenc+nvdec');
  ...
  for (const modo of candidatos) {
    ...
    notas.push({ modo, ...(await rodarProva(argsProva, duracaoSegundos)) });
  }

  const resumo = notas
    .map((n) => `${n.modo} ${n.erro ? 'falhou' : `${n.velocidade.toFixed(1)}x`}`)
    .join(', ');

  const validas = notas.filter((n) => !n.erro && n.velocidade > 0);
  if (validas.length === 0) {
    throw new Error(`prova de velocidade: nenhum modo funcionou (${notas.map((n) => `${n.modo}: ${n.erro}`).join('; ')})`);
  }
  const vencedor = validas.reduce((a, b) => (b.velocidade > a.velocidade ? b : a));
  console.log(`[reencode] prova de velocidade de ${rel}: ${resumo} -> ${vencedor.modo}`);
...
function rodarFfmpeg(args, duracaoSegundos, rel) {
  return new Promise((resolve, reject) => {
    ...
    proc.stdout.on('data', (chunk) => {
      ...
      console.log(`[reencode] ${rel}: ~${pct}%${extra}`);
      ultimoLog = Date.now();
    });

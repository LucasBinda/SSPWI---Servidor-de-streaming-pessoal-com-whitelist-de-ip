...
async function escolherModo(origem, codecOrigem, duracaoSegundos, settings, rel) {
  ...
  if (!gpuFuncionou) {
    ...
    console.log(`[reencode] prova de velocidade de ${rel}: ${resumoProva(notas)} -> software (sem GPU, prova pulada)`);
    return 'software';
  }
  ...
  console.log(`[reencode] prova de velocidade de ${rel}: ${resumoProva(notas)} -> ${vencedor.modo}`);

...
// Este módulo é o que SOBROU da reversão do HLS on-the-fly (ver
// docs/adr-001-reversao-hls.md): só as operações de mídia baratas em CPU —
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      ...
      try {
        ...
        resolve({ audio, subtitles, duracao });
      } catch (parseErr) {
        reject(parseErr);
      }
    });
...
function getSubtitle(videoPath, subIndex) {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    const args = ['-y', '-i', videoPath, '-map', `0:s:${subIndex}`, '-c:s', 'webvtt', tmpPath];
...
module.exports = { probeTracks, getSubtitle, forgetVideo };

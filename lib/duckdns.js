  ...
  atualizarDuckdns(duckdnsDominio, duckdnsToken).then((resultado) => {
    if (!resultado.ok) {
      ...
      if (estado !== estadoAnterior) {
        console.warn(`[duckdns] falha ao atualizar ${nome}: ${resultado.erro}`);
      }
      estadoAnterior = estado;
      return;
    }
    if (resultado.mudou || estadoAnterior.startsWith('erro')) {
      console.log(`[duckdns] ${nome} -> ${resultado.ips}`);
    }
    estadoAnterior = 'ok';
  });
}
...
function iniciarAtualizadorDuckdns() {
  ...
  if (duckdnsDominio && duckdnsToken) {
    console.log(`[duckdns] DNS dinâmico ativo para ${normalizarDominio(duckdnsDominio)}.duckdns.org (aponte a cada 5min)`);
  }
  ciclo();
